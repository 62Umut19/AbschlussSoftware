<template>
  <v-container class="my-5">
    ...
    <v-dialog v-model="dialog" max-width="460">
      <v-card>
        <v-card-title>Sind Sie sich sicher das Sie am {{terminId.datum}} um {{terminId.uhrzeit}} den Termin reservieren wollen?</v-card-title>
        ...
        <v-card-actions>
          ...
          <v-btn color="green darken-1" text @click="reservieren(terminId)">reservieren</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    ...
    <div class="terminplan">
      ...
      <h4 class="grey--text">{{kuerzel.vorname}} {{kuerzel.nachname}}</h4>
      <br />
    </div>

    <v-card flat v-for="uhrzeit in uhrzeiten" :key="uhrzeit.status">
      <v-layout row wrap :class="`pa-3 status ${uhrzeit.status}`">
        ...
        <v-flex xs2 sm2 md3 lg3 xl3>
          ...
          <div class="mx-4" v-for="r in raum" :key="r.raum">{{r.raum}}</div>
        </v-flex>
        <v-flex xs2 sm2 md3 lg3 xl3>
          <div class="caption grey--text">Reservieren</div>
          <div v-if="uhrzeit.status == 'frei' ">
            <v-tooltip top>
              <template v-slot:activator="{ on }">
                <v-btn text v-on="on" @click="terminId = uhrzeit" @click.stop="dialog = true">
                  <v-icon>mdi-book</v-icon>
                </v-btn>
              </template>
              <span>Jetzt den Button anklicken um die reservierung einzutragen</span>
            </v-tooltip>
          </div>
          <div v-else>
            <v-btn text disabled>
