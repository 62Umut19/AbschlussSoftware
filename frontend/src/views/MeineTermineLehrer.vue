    ...
    <div v-if="termine">
      <v-card flat v-for="termin in termine" :key="termin.terminId">
        <v-layout row wrap class="pa-3 termin">
          ...
          <v-flex xs2 sm2 md2 lg2 xl2>
            <div class="caption grey--text mb-1 mx-4">Schüler</div>
            <div class="mx-4">{{ termin.name }}</div>
          </v-flex>
          <v-flex xs2 sm2 md2 lg2 xl2>
            <div class="caption grey--text mb-1 mx-4">Klasse</div>
            <div class="mx-4">{{ termin.klasse }}</div>
          </v-flex>
          <v-flex xs2 sm2 md2 lg2 xl2>
...
export default {
  ...
  async created() {
    if (!this.$store.getters.istEingeloggtLehrer && !this.$store.getters.istEingeloggtLehrerBuchAdmin && !this.$store.getters.istEingeloggtLehrerAdmin) {
  ...
  methods: {
    async getMeineTermine() {
      try {
        console.log()
        const credentails = {
          lehrerkuerzel: this.$store.getters.getLehrer.lehrerkuerzel
        };

        const response = await GetData.getMeineTermineLehrer(credentails);
    ...
    async TerminStonieren() {
      try {
        ...
        const credentails = {
          schuelerid: this.terminid.schuelerid,
